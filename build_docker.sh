...
(DOCKER_BUILDKIT=1 docker build . --build-arg PLATFORM=linux/amd64 --build-arg VERSION=15.1 -t key2asset/postgres:${TAG} 2>&1 | tee build_docker.log) || (echo  "build failed" && exit 1)
